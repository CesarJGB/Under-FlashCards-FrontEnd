...
export default function PublicMateriaPage({ shareId }) {
  ...
  useEffect(() => {
    ...
    const loadProfile = async () => {
      ...
      try {
        ...
        if (!res.ok) {
          throw new Error(body.error || 'No se pudo cargar el perfil público.');
        }
        ...
        if (!cancelled) {
          setError(err.message || 'No se pudo cargar el perfil público.');
          setData(null);
        }
      } finally {
        if (!cancelled) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-50">
      <div className="max-w-5xl mx-auto px-4 py-8 md:px-6 md:py-10 space-y-8">
        ...
        {loading ? (
          <div className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-12 text-center shadow-xs">
            ...
            <p className="text-sm font-bold text-slate-700 dark:text-slate-200">Cargando materia publica...</p>
          </div>
        ) : error ? (
