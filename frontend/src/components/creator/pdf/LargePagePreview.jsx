  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.warn('[LargePagePreview.v1] Error rendering page:', error);
        if (!cancelled) {
          setLoading(false);
          setRenderError(true);
        }
        ...
        try {
          ...
          // Cleanup is best effort when the PDF is replaced.
        }
      }
    })();
      ...
      <div className="flex max-h-[85vh] w-full max-w-lg flex-col overflow-hidden rounded-2xl bg-white shadow-2xl animate-[scaleUp_0.15s_ease]" role="dialog" aria-modal="true" aria-label={`Vista previa de la página ${pageNum}`}>
        <div className="flex shrink-0 items-center justify-between border-b border-slate-100 bg-slate-50 px-4 py-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-bold text-slate-700">Página {pageNum}</span>
          </div>
          <button type="button" onClick={onClose} className="cursor-pointer rounded-xl p-1.5 text-slate-400 transition-all hover:bg-slate-200/60 hover:text-slate-700" aria-label="Cerrar vista previa">
