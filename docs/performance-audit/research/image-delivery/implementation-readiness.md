...
- **Corte 5A — TERMINADO** (Fase 1G, [phase-1g-cut-5a-report.md](./phase-1g-cut-5a-report.md)): telemetría temporal del contrato legacy (utilidad pura + instrumentación de las cinco lecturas + 21 pruebas deterministas; contrato legacy intacto; contract tests 51/51 y `test:image-delivery` 74/74 en verde). **OBSERVATION NOT STARTED**: la ventana de 14 días comienza tras el despliegue.
- **Corte 5B — BLOCKED** (requiere aprobar los 14 días de observación con cero peticiones legacy y tráfico real representativo). IMG-RENDER permanece PARTIAL — PENDING — DEVICE REQUIRED.
