...
- Bloqueo preexistente documentado: 5 tests backend (`aiService.test.js`, `deckRecovery.test.js`) fallan por configuración de modelo IA del entorno, idénticos en el HEAD anterior `b0b36e6` y sin cambios en `e4d86aa`, `0921717`, en la Fase 1E ni en la Fase 1F.
- **Cortes 5 — NO implementado** (requiere aprobación humana y métricas sostenidas de tráfico legacy ≈ 0). IMG-RENDER permanece PARTIAL — PENDING — DEVICE REQUIRED.
...
| Implementación | Cortes 0, 1 y 2 TERMINADOS (Fases 1C/1D/1E); Corte 3 OMITIDO deliberadamente; Corte 4 (limitado) HERRAMIENTA TERMINADA — migración NOT RUN (Fase 1F); Corte 5 pendiente de aprobación humana |
