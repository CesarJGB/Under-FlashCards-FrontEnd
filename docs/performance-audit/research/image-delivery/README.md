...
- [phase-1f-cut-4-report.md](./phase-1f-cut-4-report.md): cierre de la Fase 1F — Corte 4 limitado terminado (herramienta de normalización segura de `cardBackgrounds`: planificador puro, script `migrate:image-backgrounds` con dry-run y transacciones, 45 pruebas deterministas; Corte 3 omitido deliberadamente; **migración de producción NOT RUN**).
- [phase-1g-cut-5a-report.md](./phase-1g-cut-5a-report.md): cierre de la Fase 1G — Corte 5A terminado (telemetría temporal y segura del contrato legacy en las cinco lecturas: una línea JSON estable por petición, sin PII ni contadores; ventana de observación de 14 días **OBSERVATION NOT STARTED**; **Corte 5B BLOCKED** hasta aprobar la ventana; contrato legacy intacto).
...
Los detalles, cifras y matices por alternativa están en [recommended-architecture.md](./recommended-architecture.md). El **Corte 1 está implementado** (Fase 1D, [phase-1d-cut-1-report.md](./phase-1d-cut-1-report.md)): lecturas negociadas con `?contract=indexed`, contrato legacy intacto por defecto, resolver productivo en el frontend y resumen de mazo sin `cardBackgrounds`. El **Corte 2 está implementado** (Fase 1E, [phase-1e-cut-2-report.md](./phase-1e-cut-2-report.md)): contrato ligero `?contract=indexed&cover=thumbnail` con `coverImageThumb` opcional (generada en el frontend, sin migración ni backfill; fallback a `coverImage` para mazos antiguos). El **Corte 4 limitado está implementado como herramienta** (Fase 1F, [phase-1f-cut-4-report.md](./phase-1f-cut-4-report.md)): normalización segura de `cardBackgrounds` (planificador puro, script `migrate:image-backgrounds` con dry-run y transacciones, pruebas deterministas) — **la migración de producción queda NOT RUN**. El **Corte 3 se omitió deliberadamente** (assets/almacenamiento externo). El **Corte 5A está implementado** (Fase 1G, [phase-1g-cut-5a-report.md](./phase-1g-cut-5a-report.md)): observabilidad temporal del contrato legacy (una línea JSON por petición en las cinco lecturas, sin PII) para abrir una ventana de 14 días tras el despliegue; **OBSERVATION NOT STARTED**; el **Corte 5B** (limpieza del contrato heredado) sigue **BLOCKED** hasta aprobar esa ventana con cero peticiones legacy.
