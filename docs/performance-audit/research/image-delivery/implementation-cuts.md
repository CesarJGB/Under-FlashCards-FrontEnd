...
- Migración: ninguna (sólo contrato). Compatibilidad: **fin de soporte de clientes viejos** — requiere la ventana del 5A aprobada: 14 días completos con cero peticiones legacy y tráfico indexado real representativo en las superficies activas.
- Pruebas: contract tests del shape final; suite completa.
- Métricas (gate del 5B): **14 días completos** con **cero peticiones legacy** en las cinco rutas; **tráfico indexado positivo** en las tres superficies activas (`deck-list`, `deck-cards`, `all-cards`); **ausencia estática de consumidores productivos** en las dos rutas dormidas (`continuous-session`, `normal-session`) verificada por búsqueda al cierre; **no se exige tráfico positivo** en las rutas dormidas; **comprobación funcional de las escrituras** (crear, editar, borrar, importar, lotes) aunque no produzcan eventos 5A; suite verde.
