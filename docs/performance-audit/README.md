...
- **Fase 2B — [phase-2b-browser-profile.md](./research/library-scale/phase-2b-browser-profile.md)**: el perfil real de navegador cerró los NOT MEASURED de 2A sobre los mismos datos (HEAD `ecb0259`, `raw-results-2b.json` sanitizado): build de producción Vite + Chromium de Playwright en contenedor de investigación, 5 repeticiones válidas por escenario (B1 arranque frío, B2 arranque caliente con siembra de `localStorage`, B3 entrada a Library fría/caliente hasta "Library useful", B4 filtros/búsquedas/ordenamientos/conteos sobre los 29 mazos, B5/B6/B7 aperturas C20/C100/C500 exactas con reapertura caliente; p95 NOT MEASURED). Se separaron red (TTFB/descarga/`Content-Encoding`/tamaños), `JSON.parse`, transformación, hidratación de `safeLocalStorage`, renders/commits de React (build de profiling aparte, sin mezclar), DOM, memoria, long tasks y atribución scripting/layout/paint vía CDP. Guardia del Corte 5A con cero eventos legacy y cero escrituras. La instrumentación productiva es inerte con bandera y queda eliminada del build normal (tree-shaking). Se corrigieron defectos reales del harness (mayúsculas CSS en marcadores, mazos sin `materiaId`, título por substring, etiqueta de ordenamiento, memoria precisa en headless).
- **Corte 5A**: continúa desplegado y en observación (ventana de 14 días). Las Fases 2A y 2B emitieron exclusivamente peticiones indexadas (cero `legacy-missing`/`legacy-other`).
