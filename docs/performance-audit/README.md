...
- [research/library-scale/](./research/library-scale/README.md): Fase 2A — baseline real de escala de Library y apertura de mazos (`backend/scripts/performance/libraryScaleBaseline.js` + 54 pruebas). **MEASURED** sobre el entorno productivo autorizado (HEAD `ef8c4d0`): 29 mazos/5.877 tarjetas (máx. 545), API indexada 9.4–304 KB con medianas de 145.55–245.74 ms (C20–C500) y 235.86 ms en la lista (5 muestras/caso, p95 NOT MEASURED), explain con COLLSCAN en la lista, muestra de conteos de los 3 mazos seleccionados cubierta con `deckId_1` y sort en memoria por mazo; cero eventos legacy; `raw-results.json` sanitizado. Fase 2B — perfil real de navegador de Library y apertura de mazos ([phase-2b-browser-profile.md](./research/library-scale/phase-2b-browser-profile.md), `frontend/tests/performance/library-browser/` + 54 pruebas): **MEASURED** en Chromium sobre los mismos datos reales (B1–B7, 5 muestras/caso, p95 NOT MEASURED; `applicationBaseSha ecb0259…`, `harnessSha 611ffb0…`), con red (correlación CDP uno a uno), body/`JSON.parse` puro/transformación, hidratación de `safeLocalStorage`, renders/commits de React por escenario y repetición (build de profiling aparte), DOM, memoria, long tasks y atribución scripting/layout/paint; cero eventos legacy/all-cards/escrituras; `raw-results-2b.json` y `raw-results-2b-profiling.json` sanitizados.
...
## Estado de la Fase 2A/2B (baseline de Library) y de los cortes de imagen

- **Fase 2A — [research/library-scale/](./research/library-scale/README.md)**: el baseline real de escala de Library y apertura de mazos se ejecutó con éxito sobre el entorno productivo autorizado (contenedor backend vía Docker con la URI como variable temporal; dominio documentado como `production-backend-A`). Resultados **MEASURED** (HEAD `ef8c4d0`, `raw-results.json` sanitizado): 29 mazos propios / 5.877 tarjetas (mediana 100, p95 519.5, máx. 545; 9 mazos ≥500); API indexada 9.433–303.769 B con medianas de 145.55 ms (C20), 148.08 ms (C100), 245.74 ms (C500) y 235.86 ms (lista), 5 muestras por caso (p95 NOT MEASURED); explain con COLLSCAN+sort en memoria en la lista de mazos, muestra de conteos de los 3 mazos seleccionados cubierta con `deckId_1` (el aggregate completo de Library queda sin explain: NOT MEASURED) y apertura por mazo indexada con sort en memoria. `Content-Encoding` estuvo ausente en las solicitudes del harness, cuyo cliente Node no anunció compresión (navegador/edge NOT MEASURED). Cero eventos legacy en la telemetría del 5A. Tres defectos del harness detectados en ejecución real se corrigieron con pruebas. Quedaron sin medir los costes de navegador (render, storage, parseo real del cliente).
- **Fase 2B — [phase-2b-browser-profile.md](./research/library-scale/phase-2b-browser-profile.md)**: el perfil real de navegador cerró los NOT MEASURED de 2A sobre los mismos datos (aplicación `ecb0259…`; harness `611ffb0…`, Commit A; `raw-results-2b.json` y `raw-results-2b-profiling.json` sanitizados): build de producción Vite + Chromium de Playwright en contenedor de investigación, 5 repeticiones válidas por escenario (B1 arranque frío, B2 arranque caliente con siembra de `localStorage`, B3 entrada a Library fría/caliente hasta "Library useful", B4 filtros/búsquedas/ordenamientos/conteos sobre los 29 mazos, B5/B6/B7 aperturas C20/C100/C500 exactas con reapertura caliente; p95 NOT MEASURED). Se separaron y persistieron por escenario y repetición: red (CDP con correlación uno a uno, sin reutilización de entradas), body/`JSON.parse` puro/transformación, hidratación de `safeLocalStorage`, renders/commits de React (build de profiling aparte, B1–B7), DOM, memoria, long tasks y atribución scripting/layout/paint vía CDP (B4: NOT MEASURED). Guardia del Corte 5A con cero eventos legacy, cero all-cards y cero escrituras (160 indexadas por run). La instrumentación productiva es inerte con bandera (desactivada y no-op en el build normal; no se afirma eliminación por tree-shaking). Estado: **PASS PARCIAL** (p95 y atribución CPU de B4 en `notMeasured`). Se corrigieron defectos reales del harness (mayúsculas CSS en marcadores, mazos sin `materiaId`, título por substring, etiqueta de ordenamiento, memoria precisa en headless) y los defectos de integridad de la medición (correlación CDP, persistencia por repetición, flujo de dos commits).
- **Corte 5A**: continúa desplegado y en observación (ventana de 14 días). Las Fases 2A y 2B emitieron exclusivamente peticiones indexadas (cero `legacy-missing`/`legacy-other`, cero all-cards, cero escrituras).
...
- Las Fases 2A y 2B **no implementan optimizaciones productivas**; 2B añade únicamente instrumentación inerte con bandera de investigación, sin modificar flujos, dependencias ni contratos.
