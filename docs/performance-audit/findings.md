...
| PERF-IMG-001 | Imágenes/datos | Crítica | Confirmada | `Deck.js:11-12,38-66`; `Flashcard.js:58-79`; `getCardsByDeck` | respuestas/heap/storage se disparan; riesgo de límites duros | Data URL en Mongo; fondo compartido expandido en cada tarjeta; fondos también en lista de mazos | multiplicación por n en red/parse/estado y crecimiento documental | mazo con un fondo y 20/100/500/1000 tarjetas; inspeccionar JSON | medir bytes únicos/repetidos; evaluar referencias/URLs/thumbnails como opciones |
...
| PERF-IMG-003 | Grid/hover | Alta | Fuerte | `FlashcardGrid:12-42,123-202`; `DeckCard:32-39` | lag reportado al pasar/interactuar con tarjetas con fondo | grid completo + fondos CSS + overlays + sombra; presión de superficies | pintura/composición/memoria crece con tarjetas/resolución | capturar Paint/Layers/Timelines con fondos | aislar variables; no retirar sombra ni memoizar sin perfil |
...
**Reproducción/medición.** Crear fuera de producción un Deck con un único fondo de tamaño conocido y 20/100/500/1000 tarjetas; capturar `encodedDataLength`, tamaño transferido/descomprimido, tiempo de parseo, heap y longitud de `JSON.stringify`. Comparar bytes totales con bytes únicos por hash. No se ejecutó por ausencia de dataset.
...
**No confirmado.** El pseudoestado hover por sí solo no hace un `setState` React, así que la creación de objetos `style` no explica directamente cada movimiento del puntero. Tampoco está demostrado si la sombra provoca paint costoso, si se promocionan capas, si el motor comparte la superficie decodificada de Data URL idénticas o si ocurre redecodificación. Atribuirlo sólo a `hover:shadow-md` sería incorrecto.
...
**Reproducción.** Usar cuatro datasets idénticos salvo por una variable: sin fondo; fondo pequeño compartido; fondo grande compartido; fondos grandes distintos. Repetir 20/100/500/1000. Capturar interacción de puntero, apertura del menú y scroll con Performance, paint flashing, Layers, memoria y Safari Timelines. Medir INP/event duration, frames perdidos, paint/composite, long tasks, heap y cantidad de superficies. No hay captura disponible en esta auditoría.
