...
| `EDITOR-COLOR-001` | B | Activación única: puntero abre en `pointerdown` primario con `preventDefault`; teclado/AT abre por click semántico. `showPicker()` usa feature detection y `input.click()` es fallback en la misma activación. | SES + `StylePanel` + `editorActivation` | 1 + corrección profunda posterior a Corte 5 | `UT-ACT-001/002`, `UT-PICK-004`, `PW-PICK-001`, `DEV-IOS-002`, `DEV-AND-001` | click tardío que permitió transferencia de foco y no abrió en dispositivo | A | Cubierto; certificación pendiente |
