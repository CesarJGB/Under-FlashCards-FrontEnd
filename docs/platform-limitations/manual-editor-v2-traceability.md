...
| `EDITOR-COLOR-001` | B | El control custom es el `input[type=color]` nativo transparente que recibe directamente el gesto. La transacción nace antes de `input/change/cancel`; no usa `showPicker()` ni `input.click()` programáticos. | SES + `StylePanel` | corrección Safari/iOS posterior a Corte 5 | contratos estáticos, `PW-PICK-003`, `DEV-IOS-002`, `DEV-AND-001` | botón intermediario y color input `sr-only` activado programáticamente | A | Cubierto en DOM; dispositivo pendiente |
| `EDITOR-COLOR-002` | B | Tratar el picker nativo como transacción externa; preservar datos/rango y ofrecer reanudación explícita, sin promesa de OSK. | SES | 1 | `UT-PICK-002`, `PW-PICK-003`, `DEV-IOS-002` | reintentos que equiparan foco con teclado conservado | A | Cubierto |
...
| `KEEP-002` | Gesto confiable abre el picker nativo | El propio input de color cubre el control visual y recibe el gesto; no depende de una activación programática que WebKit pueda rechazar. | corrección Safari/iOS; `PW-PICK-003`, `DEV-IOS-002` |
...
- **Contratos conservados:** `useKeyboardHeight.js` externo, API heredada de `scrollLock.js`, `useModalAccessibility`, textarea y selección, input color nativo no controlado, portales, scroll interno, safe areas, `-webkit-fill-available`, leases, tokens, inert, sentinel y registro global.
