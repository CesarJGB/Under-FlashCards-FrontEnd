...
| `EDITOR-COLOR-001` | B | El control custom abre un ActionSheet propio con borrador HSL/HEX y destino congelado; no usa picker nativo, `showPicker()` ni `input.click()`. | UI + `StylePanel` | corrección Safari/iOS posterior a Corte 5 | contratos estáticos, `PW-COLOR-APP-001/002`, `DEV-IOS-002` | input color nativo cuyo lifecycle desmontaba la paleta | A | Cubierto en DOM; dispositivo pendiente |
| `EDITOR-COLOR-002` | B | Tratar el color propio como borrador local: sliders/hex no mutan el modelo y Apply/Cancel son la única frontera. | UI color | seguimiento posterior | `UT-COLOR-*`, `PW-COLOR-APP-001/002`, `DEV-IOS-002` | lifecycle ambiguo del picker nativo | A | Cubierto en DOM |
...
| `EDITOR-COLOR-003` | B/D | Resolver por `input/change`, `cancel` si existe o retorno `unknown`; no esperar `blur` ni tiempo fijo. | SES + `StylePanel` | 1 | `UT-PICK-001`, `UT-PICK-002`, `PW-PICK-003` | secuencia `change → blur → 80 ms` | A | Cubierto |
...
| `KEEP-002` | Primer gesto produce una transición confiable | El botón semántico abre el sheet propio; el file input real recibe directamente el gesto posterior dentro del sheet de imagen. | corrección Safari/iOS; `PW-COLOR-APP-001`, `PW-IMAGE-APP-001`, dispositivo pendiente |
| `KEEP-003` | La edición continua no confirma prematuramente | El color vive en un borrador local estable; sliders/hex no desmontan el sheet ni mutan la tarjeta. | `UT-COLOR-*`, `PW-COLOR-APP-001/002`, `DEV-IOS-002` |
...
- **Contratos conservados:** `useKeyboardHeight.js` externo, API heredada de `scrollLock.js`, `useModalAccessibility`, textarea y selección, portales, scroll interno, safe areas, `-webkit-fill-available`, leases, tokens, inert, sentinel y registro global. El input color nativo quedó sustituido por el borrador propio tras la prueba física.
