...
| `apple-mobile-web-app-status-bar-style` | Extensión Apple para web apps Home Screen | iOS standalone; requiere `apple-mobile-web-app-capable=yes` | `default` mantiene el contenido bajo la barra; solo `black-translucent` permite que la superficie web ocupe toda la pantalla detrás de la status bar | Usar `black-translucent` junto a `viewport-fit=cover`; dejar que el propietario existente de safe area proteja el contenido y validar color/contraste en PWA física | Alta | 2026-08-22 |
| `<meta name="theme-color">` | Meta HTML soportado por Safari 15+ | Safari 15+ documenta color de status bar en iOS y chrome/overscroll en otras plataformas Apple | No sustituye el modo edge-to-edge de Apple; versiones de Safari pueden decidir el tinte usando también la superficie que llega al borde superior | Durante una superficie temporal, sincronizar el único meta existente con `html`/`body`/`#root` y restaurar todos sus valores; no usarlo como mecanismo único | Media | 2026-08-22 |
...
| `100dvh` | Unidad dinámica estándar | iOS 15.4+; CA/WV 108+; SI 21+ | Cambia con chrome dinámico; con el modo de teclado predeterminado no representa necesariamente el área sobre el OSK; en un cold launch real de iOS standalone se observó que permanecía temporalmente corto después de que el documento y `100vh` ya ocuparan la superficie completa | `min-height:100vh; min-height:100dvh` y medición visual solo cuando sea necesaria; para la superficie fullscreen de Login, `100vh` únicamente bajo WebKit + standalone | Alta | 2026-08-22 |
| `100svh` | Unidad de viewport pequeño estándar | iOS 15.4+; CA/WV 108+; SI 21+ | Reserva el peor caso de UI y puede dejar espacio libre cuando se retrae; WebKit 254868 registra que en una PWA instalada con `viewport-fit=cover` puede excluir la safe area | Útil para contenido siempre visible; en el root standalone usar el fallback acotado `100vh` mientras el bug siga abierto | Alta | 2026-08-22 |
...
| `-webkit-fill-available` | Extensión WebKit de sizing disponible en Safari | WebKit; no es una unidad estándar portable | En PWAs instaladas con `viewport-fit=cover`, WebKit 254868 puede devolver la altura disponible menos la safe area y dejar el root corto | Mantenerla como fallback de Safari normal; sobrescribir el `min-height` del root con `100vh` solo en `display-mode: standalone` | Media | 2026-08-22 |
