...
| VP-09 · Login en arranque frío de Safari Home Screen | Evitar un primer frame con safe area duplicada | 1. Cerrar completamente la PWA.<br>2. Lanzarla desde Home Screen sin sesión.<br>3. Grabar desde antes de aparecer Login hasta dos frames estables.<br>4. Comparar rectángulos de `<main>`, header/logo y botón “Iniciar sesión”.<br>5. Abrir/cerrar el ActionSheet y repetir tras volver a Login si el flujo lo permite. | El primer frame visible coincide con el estable: no hay desplazamiento vertical, `body` ya tiene padding computado cero, Login conserva sus insets propios y ActionSheet no altera la restauración. Registrar por separado cualquier carga tardía de fuente o imagen. |
