...
| VP-08 · Safari Home Screen con Home Indicator | Validar workaround WK-254868 y propietario del inset inferior | 1. Con la PWA recién instalada, registrar `screen.height`, `innerHeight`, `documentElement.clientHeight`, `visualViewport.height/offsetTop/scale` y los rectángulos de `html`, `body`, `#root`, shell y navbar.<br>2. Comparar la build anterior y la build con el override standalone.<br>3. Confirmar que el incremento de `html/body/#root` coincide con la franja eliminada.<br>4. Abrir y cerrar un input/textarea tres veces y repetir el registro tras estabilizar.<br>5. Abrir la misma build en Safari normal. | En standalone el fondo llega al borde físico, sin cambiar altura/posición visual de navbar ni sumar otro inset. Tras cerrar OSK la geometría estable vuelve exactamente al baseline. Safari normal no activa el override. Si la diferencia no coincide con la franja, no atribuirla a WK-254868. |
| VP-09 · Login en arranque frío de Safari Home Screen | Evitar la superficie corta mientras WebKit estabiliza el viewport | 1. Cerrar completamente la PWA.<br>2. Lanzarla desde Home Screen sin sesión y con `viewportDebug` activo.<br>3. Grabar desde antes de aparecer Login hasta dos frames estables.<br>4. Comparar rectángulos de `<main>`, `html`, `body`, `#root`, header/logo y botón “Iniciar sesión”, junto con `100vh`, `100dvh` y VisualViewport.<br>5. Abrir el ActionSheet, enfocar el código, cerrar el OSK y comprobar que vuelve al baseline.<br>6. Rotar portrait → landscape → portrait y repetir el cold launch cuando sea viable. | Durante toda la secuencia Login coincide con la superficie del documento, incluso si `100dvh` y VisualViewport permanecen temporalmente más cortos que `100vh` y `#root`. No hay franja ni salto; `body` conserva padding computado cero, Login mantiene sus insets propios y el sheet recupera snap/layout tras cerrar OSK. Registrar por separado cualquier carga tardía de fuente o imagen. |
| LD-01 · iPhone Safari y Home Screen, ambas orientaciones | Detectar línea fina en la composición del MP4 de carga | 1. Cargar una sesión que muestre la pantalla de carga.<br>2. Grabar el vídeo completo en portrait y landscape.<br>3. Repetir tres veces en Safari normal y tres en Home Screen.<br>4. Comparar los bordes superior e inferior del frame, con toolbar expandida y retraída cuando aplique. | El frame conserva su relación cuadrada y no aparece una línea negra en ningún borde. Registrar por separado cualquier artefacto que solo aparezca durante `onEnded` o al cambiar de pantalla. |
| LD-02 · iPhone Home Screen con Dynamic Island, portrait, físico | Validar status bar y secuencia exclusiva de marca | 1. Instalar la PWA y lanzarla desde el icono.<br>2. Grabar desde Lua hasta Home.<br>3. Comprobar por frames `icono → salida → logo completo → reveal`.<br>4. Comparar el fondo detrás de hora, Dynamic Island, Wi-Fi y batería durante cada fase y al terminar.<br>5. Navegar por Home y relanzar la PWA. | Nunca coinciden icono PWA y logo completo. La zona superior usa `#FBFAFF` en Lua y el mismo `#EDE9FE` durante toda la superficie lila, sin franja ni flash; al terminar recupera el aspecto normal de Home y no conserva el lila al navegar/relanzar. Los indicadores son legibles y no hay doble inset ni desplazamiento de Home. |
