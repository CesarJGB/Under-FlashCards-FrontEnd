...
Desde la corrección del contrato de controles flotantes, ese offset inferior pertenece a `DashboardBottomDock`: el host de controles y la navbar móvil son dos filas del mismo flujo, separadas por un único `gap`. La altura renderizada de la navbar desplaza el host directamente; `LibraryFAB`, `ExamFAB` y la CTA de selección de mazos portan al host y no calculan su posición con `vh`, `dvh`, safe area ni una copia de la altura del navbar. El host conserva por separado el offset desktop anterior. `PdfExtractor` aplica la misma composición respecto al footer real de `FlashcardCreator`, dentro de la pantalla inmersiva de edición. Cuando la ruta de calendario oculta la navegación global, `ScheduleListScreen` y `ScheduleMobileFooter` conservan su contrato inmersivo y no consumen el dock global. Los paneles de depuración condicionados a `DEV` quedan fuera de este contrato para no modificar la instrumentación durante una corrección de producto.
