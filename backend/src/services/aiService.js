...
// Compatibilidad con clientes que todavía envíen nombres del selector anterior.
const OPENROUTER_ROUTING_ALIASES = Object.freeze({
  speed: OPENROUTER_ROUTING_MODE.SPEED,
  latency: OPENROUTER_ROUTING_MODE.SPEED,
  auto: OPENROUTER_ROUTING_MODE.ECONOMIC,
  economy: OPENROUTER_ROUTING_MODE.ECONOMIC,
  economic: OPENROUTER_ROUTING_MODE.ECONOMIC,
});
const OPENROUTER_DEFAULT_ROUTING_MODE = OPENROUTER_ROUTING_MODE.SPEED;
...
function normalizeOpenRouterRoutingMode(value, fallback = OPENROUTER_DEFAULT_ROUTING_MODE) {
  ...
  if (OPENROUTER_ROUTING_MODES.includes(normalized)) return normalized;
  return OPENROUTER_ROUTING_ALIASES[normalized] ?? fallback;
}

function getOpenRouterProviderSort(routingMode = OPENROUTER_DEFAULT_ROUTING_MODE) {
  return normalizeOpenRouterRoutingMode(routingMode);
}
...
function createOpenRouterRequestBody(requestBody = {}, { routingMode } = {}) {
  ...
  return {
    ...
    provider: {
      ...
      ...OPENROUTER_PROVIDER_PERFORMANCE,
      sort: providerSort,
    },
  };
}
...
// OpenRouter trata estos valores como preferencias: los endpoints que no las
// cumplen se envían al final, pero siguen disponibles como fallback.
// P50 = rendimiento típico del lote; P90 = guardarraíl de latencia de cola.
const OPENROUTER_P50_MIN_THROUGHPUT = readBoundedNumber(
  process.env.OPENROUTER_P50_MIN_THROUGHPUT,
  60,
  0,
  100000
);
const OPENROUTER_P90_MAX_LATENCY = readBoundedNumber(
  process.env.OPENROUTER_P90_MAX_LATENCY,
  3,
  0.1,
  3600
);
const OPENROUTER_PROVIDER_PERFORMANCE = Object.freeze({
  preferred_min_throughput: Object.freeze({
    p50: OPENROUTER_P50_MIN_THROUGHPUT,
  }),
  preferred_max_latency: Object.freeze({
    p90: OPENROUTER_P90_MAX_LATENCY,
  }),
});
...
async function callOpenRouterJson({ apiKey, requestBody, context = {}, parseResponse, signal }) {
  ...
  const routerLogContext = {
    ...
    performanceGuardrails: OPENROUTER_PROVIDER_PERFORMANCE,
...
module.exports = {
  ...
  OPENROUTER_PROVIDER,
  OPENROUTER_PROVIDER_PERFORMANCE,
  OPENROUTER_P50_MIN_THROUGHPUT,
  OPENROUTER_P90_MAX_LATENCY,
  OPENROUTER_ROUTING_ALIASES,
